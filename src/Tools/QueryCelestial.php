...
DESC;
    ...
    public function handle(Request $request): string
    {
        ...
        try {
            return match ($action) {
                'moon_phase' => $this->service->moonPhase($date, $timezone),
                'sun_info' => $this->service->sunInfo($date, $lat ?? 0, $lon ?? 0, $timezone),
                'moon_info' => $this->service->moonInfo($date, $lat ?? 0, $lon ?? 0, $timezone),
                'planet_position' => $this->service->planetPosition($planet, $date, $lat ?? 0, $lon ?? 0, $timezone),
                'solar_eclipse' => $this->service->solarEclipse($date ?? date('Y-m-d'), $lat ?? 0, $lon ?? 0),
                'lunar_eclipse' => $this->service->lunarEclipse($date ?? date('Y-m-d')),
                'night_sky' => $this->service->nightSky($lat ?? 0, $lon ?? 0, $timezone),
                'zodiac_report' => $this->service->zodiacReport($date),
                'time_info' => $this->service->timeInfo($date),
